...
4. Tightens live length with residual MITM on 4×4 / 5×5.

This *is* the algorithm for solving any size greater than 3. It is not a closed-form exact g(n) — none exists in the literature.
...
ClusterScheduler
  ...
  → BoundHarness (SSTM + OBTM + per-stage OBTM vs U(n))
```
...
| n | U(n) | Community / estimates |
|---|------|------------------------|
| 4 | 501 | OBTM upper **54**; probabilistic ~41 HTM / ~48 QTM |
| 5 | 878 | OBTM ~130 claimed; probabilistic ~58 HTM / ~68 QTM |
| 6 | 1321 | open |
| 7 | 1852 | open |
| 10 | 3981 | open |
...
Computing God's number is the diameter of the Cayley graph. For n=3 that required ~35 CPU-years. For n=4 the position count is ~7.4×10⁴⁵. Honest progress: keep U(n), shrink measured OBTM, raise residual MITM quality.
