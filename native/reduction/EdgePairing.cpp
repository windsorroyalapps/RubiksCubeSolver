...
static constexpr int kStageEdges[4] = {0, 2, 4, 6};
...
static std::vector<Move> pairRemaining(Cube& work, bool allowCenterDamage, bool protectStage);
static std::vector<Move> restoreCentersHoldEdges(Cube& work);

// NOTE: full implementations follow — see commit body for hybrid pairing logic.
...
static bool improveStageCap(Cube& work, std::vector<Move>& out,
    ...
    bool found = false;
    int bestSc = scBefore, bestC = cBefore + (allowCenterDamage ? 64 : 0);
    int bestSolid = solidBefore, bestPaired = pairedBefore, bestLen = 999;
    ...
    for (const auto& seq : candidates) {
        ...
        if (!allowCenterDamage) { if (c > cBefore) continue; }
        else {
        ...
        if (sc == scBefore && c >= cBefore && sol <= solidBefore && paired <= pairedBefore) continue;
        const bool better = !found || sc < bestSc || (sc == bestSc && c < bestC) ||
        ...
        if (better) {
            found = true; bestSc = sc; bestC = c; bestSolid = sol; bestPaired = paired;
            bestLen = len; bestSeq = seq;
        }
    }
    if (!found) return false;
    for (const Move& m : bestSeq) { work.apply(m); out.push_back(m); }
    return true;
}
...
static bool bfsStageCap(Cube& work, std::vector<Move>& out, int maxDepth, size_t nodeCap,
    ...
    for (int f : kFaces) for (int t : {1, -1, 2}) gens.push_back(Move{f, 0, t});
    for (int f : kFaces) for (int t : {1, -1, 2}) for (int d = 1; d < maxSlice; ++d)
        gens.push_back(Move{f, d, t});
    struct Node { Cube cube; std::vector<Move> path; };
    std::queue<Node> q; q.push({work, {}});
    auto keyOf = [&](const Cube& c) -> uint64_t {
        uint64_t k = 1469598103934665603ULL;
        auto feed = [&](unsigned v) { k ^= static_cast<uint64_t>(v) + 0x9e3779b97f4a7c15ULL; k *= 1099511628211ULL; };
        for (int f = 0; f < 6; ++f) for (int r = 1; r < n - 1; ++r) for (int col = 1; col < n - 1; ++col)
            feed(static_cast<unsigned>(c.get(f, r, col)));
        for (int i = 1; i < n - 1; ++i) {
            feed(static_cast<unsigned>(c.get(U, n - 1, i))); feed(static_cast<unsigned>(c.get(F, 0, i)));
            feed(static_cast<unsigned>(c.get(U, 0, i))); feed(static_cast<unsigned>(c.get(B, 0, i)));
            feed(static_cast<unsigned>(c.get(D, 0, i))); feed(static_cast<unsigned>(c.get(F, n - 1, i)));
            feed(static_cast<unsigned>(c.get(D, n - 1, i))); feed(static_cast<unsigned>(c.get(B, n - 1, i)));
        }
        return k;
    };
    std::set<uint64_t> seen; seen.insert(keyOf(work));
    size_t nodes = 0; int bestSc = sc0, bestC = c0; std::vector<Move> bestPath;
    const int midCap = allowCenterDamage ? (c0 + 6) : (c0 == 0 ? 0 : c0 + 2);
    while (!q.empty() && nodes < nodeCap) {
        Node cur = std::move(q.front()); q.pop(); ++nodes;
        ...
        if (endOk && (sc < bestSc || (sc == bestSc && c < bestC))) {
            bestSc = sc; bestC = c; bestPath = cur.path;
            if (sc == 0 && c <= c0) break;
        }
        ...
        for (const Move& m : gens) {
            ...
            Cube nxt = cur.cube; nxt.apply(m);
            if (breaksStageSolid(cur.cube, nxt) && stageCapLeftover(nxt) >= sc) continue;
            if (absoluteCenterBad(nxt) > midCap) continue;
            if (stageCapLeftover(nxt) > sc0 + 3) continue;
            const uint64_t k = keyOf(nxt);
            if (seen.count(k)) continue;
            seen.insert(k);
            Node nn; nn.cube = std::move(nxt); nn.path = cur.path; nn.path.push_back(m);
            q.push(std::move(nn));
        }
    }
    if (bestPath.empty() || bestSc >= sc0) return false;
    Cube verify = work; for (const Move& m : bestPath) verify.apply(m);
    if (!allowCenterDamage && absoluteCenterBad(verify) > c0) return false;
    if (allowCenterDamage && absoluteCenterBad(verify) > c0 && stageCapLeftover(verify) >= sc0) return false;
    for (const Move& m : bestPath) { work.apply(m); out.push_back(m); }
    return true;
}
...
static std::vector<Move> solveStageCapEdges(Cube& work, bool allowCenterDamage) {
    ...
    std::stable_sort(candidates.begin(), candidates.end(),
                     [](const std::vector<Move>& a, const std::vector<Move>& b) { return a.size() < b.size(); });
    for (int round = 0; round < 200; ++round) {
        ...
        if (bfsStageCap(work, moves, sc <= 2 ? 6 : 5, sc <= 2 ? 150000 : 80000, allowCenterDamage)) continue;
        break;
    }
    return moves;
}
...
static bool improveEdge(Cube& work, int edgeIndex, std::vector<Move>& out,
    ...
    const int cBefore = absoluteCenterBad(work);
    bool found = false;
    int bestDelta = 0, bestSc = scBefore, bestC = cBefore + (allowCenterDamage ? 64 : 0), bestLen = 999;
    std::vector<Move> bestSeq;
    for (const auto& seq : candidates) {
        Cube probe = work;
        for (const Move& m : seq) probe.apply(m);
        ...
        if (sc > scBefore) continue;
        if (!allowCenterDamage) { if (c > cBefore) continue; }
        else if (c > cBefore + 8) continue;
        const int delta = EdgePairing::pairedWings(probe, edgeIndex) - before;
        if (delta <= 0) continue;
        const int len = static_cast<int>(seq.size());
        const bool better = !found || delta > bestDelta || (delta == bestDelta && c < bestC) ||
            (delta == bestDelta && c == bestC && sc < bestSc) ||
            (delta == bestDelta && c == bestC && sc == bestSc && len < bestLen);
        if (better) {
            found = true; bestDelta = delta; bestSc = sc; bestC = c; bestLen = len; bestSeq = seq;
        }
    }
    if (!found) return false;
    for (const Move& m : bestSeq) { work.apply(m); out.push_back(m); }
    ...
    for (int pass = 0; pass < 8; ++pass) {
        ...
        for (int e : order) {
            for (int attempt = 0; attempt < 40; ++attempt) {
                if (!improveEdge(work, e, moves, candidates, protectStage, allowCenterDamage)) break;
                any = true;
...
static std::vector<Move> restoreCentersHoldEdges(Cube& work) {
    ...
    for (int round = 0; round < 64; ++round) {
        ...
        if (c0 == 0) break;
        bool found = false; int bestC = c0; std::vector<Move> best;
        for (int af : kFaces) for (int bf : kFaces) {
            if (af == bf || (af ^ 1) == bf) continue;
            for (int ad = 0; ad < 2; ++ad) for (int bd = 0; bd < 2; ++bd)
                for (int at : {1, -1, 2}) for (int bt : {1, -1, 2}) {
                    Move mA{af, ad, at}; Move mB{bf, bd, bt};
                    std::vector<Move> seq = {mA, mB, inverse(mA), inverse(mB)};
                    Cube probe = work; for (const Move& m : seq) probe.apply(m);
                    if (stageCapLeftover(probe) <= e0 && absoluteCenterBad(probe) < bestC) {
                        bestC = absoluteCenterBad(probe); best = seq; found = true;
                    }
                }
        }
        if (!found) break;
        for (const Move& m : best) { work.apply(m); out.push_back(m); }
    }
    return out;
}

std::vector<Move> EdgePairing::pairOne(Cube& work, int edgeIndex, const std::bitset<12>&) {
    std::vector<Move> moves;
...
std::vector<Move> EdgePairing::pairAll(Cube& work) {
    ...
    auto rest = pairRemaining(work, false, true);
    ...
    if (stageCapLeftover(work) > 0) {
        ...
        auto rest2 = pairRemaining(work, true, true);
    ...
    // Force remaining equatorial edges once stage strips are clean.
    if (stageCapLeftover(work) == 0 && EdgePairing::leftoverUnpairedWings(work) > 0) {
        auto rest3 = pairRemaining(work, true, false);
        solution.insert(solution.end(), rest3.begin(), rest3.end());
        if (stageCapLeftover(work) > 0) {
            auto fix = solveStageCapEdges(work, true);
            solution.insert(solution.end(), fix.begin(), fix.end());
        }
    }
