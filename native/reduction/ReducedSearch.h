 ...
 * + complete residual coordinate tables remain highest leverage for OBTM ≤55.
 ...
 * remaining for collapsing constructive lengths toward community OBTM ≤55.
 ...
 * 2026-08-19: **Lift residual MITM to 5×5** — residualKey/residualCoords now support
 * n=5 (mid-edge Lehmer + orient + light center sample packing); meetInMiddle enabled
 * for n=5 with conservative nodeBudget (25k) / half-depth so mobile stays responsive
 * while desktop can raise via future JNI flag. DepthCap 14 for 5x5. This is the
...
private:
    ...
    // Compact residual fingerprint for meet-in-middle / hashing (4x4 + 5x5).
    // High bits: centers (exact 4x4 or sampled 5x5); low: denser wing orient + Lehmer.
    ...
    // within depthCap/2 each side; empty otherwise. Hardened node budget (100k 4x4 / 25k 5x5).
